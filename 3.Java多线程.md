...
# 并发工具类
## CountDownLatch
- 规定多少个阻塞事件发生，当前等待线程被唤醒
- 实现计数器功能
- countDown：计数器-1，
- await：当前线程阻塞
- 场景：飞机等待所有乘客入座后，才起飞
- 适用于所有线程通过一点后，通知某线程执行
- 继承AQS的共享锁通知进程，利用CAS进行
- 计数器不会重置
## CyclicBarrier
- 规定多少个线程在等待后，所有等待状态的线程都被唤醒
- await：当前线程阻塞
- 场景：赛跑裁判等待5个运动员准备好，鸣枪后，5个运动员开始跑
- 适用于让所有线程在同一点执行
- 采用ReentrantLock的condition通知
- 计数器会自动重置，若手动调用reset方法，存在阻塞的线程，则会报BrokenBarrierException异常
## Semaphore
- synchronized的加强版
- 可以控制线程的并发数量
# ThreadLocal
